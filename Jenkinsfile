pipeline {
    ...
    stages {
        ...
        stage('Upload in nexus repo'){
            steps{
            sh label: '', script: '''curl -v -u admin:admin123 --upload-file target/scala-2.12/HelloWorld-assembly-0.1.jar  http://10.1.100.158:8081/#admin/repository/repositories:repository-example/org/foo/1.0/artifact.jar
