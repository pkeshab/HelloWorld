pipeline {
    ...
    stages {
        ...
        stage('Upload in nexus repo'){
            ...
            echo "$WORKSPACE"
            //sh label: '', script: 'ls ${WORKSPACE}/target/scala-2.12' 
            sh "curl -v -u admin:admin123 --upload-file $WORKSPACE@2/target/scala-2.12/HelloWorld-assembly-0.1.jar  http://10.1.100.158:8081/repository/releases/org/foo/1.0/foo-1.0.jar"
            }
            }
        }
